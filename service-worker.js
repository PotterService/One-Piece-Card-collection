const SHELL='bow-one-piece-shell-v2',IMAGES='bow-one-piece-images-v2',FILES=['./','./index.html','./assets/css/styles.css','./assets/js/db.js','./assets/js/app.js','./manifest.webmanifest'];
...
if(u.hostname==='www.optcgapi.com'&&u.pathname.includes('/media/static/Card_Images/')){e.respondWith(caches.open(IMAGES).then(async c=>{const hit=await c.match(e.request);if(hit)return hit;const res=await fetch(e.request);if(res.ok)c.put(e.request,res.clone());return res}));return}
